...
def plot_pop_dens(data, avg_data, dt, dx, emin, emax, title, basis, printout=0):
    ...
    plt.ylim(0,1.0)
    plt.imshow(surface, cmap="hot", aspect = 'auto', extent=[0,(t_points*dt),emin,emax], interpolation='gaussian')
...
for decoherence_option in decoherence_options:
            ...
            for istate_index in range(num_istates):
                ...
                for i in range( 1000 ):
                    ...
                    if dynamics_option == 0:
                        ...
                        if decoherence_option_count == 3:

                            # mb dynamics
                            if basis_option_count == 0:
                                y = float( namd_data_line[ 451 ] ) - float( namd_data_line[1] )
                                if y < 0:
                                    y = 0.00
                            ...
                            # elec dynamics
                                ...
                                y = float( namd_data_line[ 1 ] ) - float( namd_data_line[1] )
                                if y < 0:
                                   y = 0.00
                            ...
                            # mb dynamics
                            if basis_option_count == 0:
                                y = float( namd_data_line[ 452 ] ) - float( namd_data_line[1] )
                                if y < 0:
                                    y = 0.00
    ...
    #xdata              = [ xdata[i] for i in range(200) ] 
    #ydata_avg_mb       = [ ydata_avg_mb[i] for i in range(200) ]
    #ydata_avg_mixed_sd = [ ydata_avg_mixed_sd[i] for i in range(200) ]
    ...
    plt.figure(num=None, figsize=(3.21, 2.41), dpi=300, edgecolor='black', frameon=True)
    plt.subplot(1,1,1)
    plt.title("Cubic, "+decoherence_options_names[decoherence_option_count], fontsize=12)
    plt.xlabel('Time, fs',        fontsize=12)
    plt.xticks(fontsize=12) 
    if dynamics_option == 0:
        plt.ylabel('Hot Carrier Energy, eV',     fontsize=12)
        plt.xticks([0,250,500,750,1000],  fontsize=12)
        plt.yticks([0.0,0.2,0.4,0.6], fontsize=12)
        plt.ylim(0,0.6)
    elif dynamics_option == 1:
        plt.ylabel('Population',     fontsize=12)
        plt.yticks([0.0,0.3,0.6],fontsize=12)
        plt.ylim(0,0.6)
    for i in range( len(dyns[decoherence_option_count][1]) ):
        plt.plot( xdata, dyns[decoherence_option_count][1][i], label="", linewidth=1, color = "indianred")
    for i in range( len(dyns[decoherence_option_count][0]) ):
        plt.plot( xdata, dyns[decoherence_option_count][0][i], label="", linewidth=1, color = "cornflowerblue")
    plt.plot(xdata, ydata_avg_mixed_sd_fit, linewidth=2, color="darkred",     label="Single-particle")
    plt.plot(xdata, ydata_avg_mb_fit,       linewidth=2, color="darkblue",      label="Many-body")
    plt.tight_layout()
    if dynamics_option == 0:
        plt.savefig("CsPbI3_mb_mixed_sd_Hot_Energy_Decay_tp0_"+decoherence_options[decoherence_option_count])
    elif dynamics_option == 1:
        plt.savefig("CsPbI3_mb_mixed_sd_Recovery_tp0_"+decoherence_options[decoherence_option_count])
