...
params["init_times"]     = [0]
params["active_space"]   = list(range(1,params["nstates"])) # indexing is from 0!
...
params["init_times"]     = [0]
params["active_space"]   = list(range(1,151))#params["nstates"])) # indexing is from 0!
# Include HOMO and up to the last electronic state
...
for traj in range( num_nuclear_trajs ):
    ...
    istates_mb_all_trajs.append( get_initial_states( energies_mb_all_trajs[ traj ], num_istates_to_generate, 0.01, 0.8, 0 ) )

    energies_mixed_sd_all_trajs.append( compute_state_energies_vs_time( hvib_mixed_sd_trajs[ traj ] ) )
    istates_mixed_sd_all_trajs.append( get_initial_states( energies_mixed_sd_all_trajs[ traj ], num_istates_to_generate, 0.01, 0.8, 0 ) )
...
def myfunc( istate_index ):
    ...
    for nuc_traj_index in range( num_nuclear_trajs ):
        ...
        outfile = "_mixed_sd_fssh_"+str( nuc_traj_index )+"_"+str( istate_index )+".txt"
